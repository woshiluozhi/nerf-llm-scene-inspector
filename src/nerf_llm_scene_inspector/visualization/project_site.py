...
class SiteRunEntry:
    ...
    failure_diagnostics_blocker_count: int
    capture_manifest_fail_count: int
    real_run_plan_blocker_count: int
    real_run_plan_warning_count: int
    result_status: str
    ...
    for raw in raw_entries[:max_runs]:
        ...
        entries.append(
            SiteRunEntry(
                ...
                failure_diagnostics_blocker_count=_safe_int(raw.get("failure_diagnostics_blocker_count")),
                capture_manifest_fail_count=_safe_int(raw.get("capture_manifest_fail_count")),
                real_run_plan_blocker_count=_safe_int(raw.get("real_run_plan_blocker_count")),
                real_run_plan_warning_count=_safe_int(raw.get("real_run_plan_warning_count")),
...
def _runs_section(entries: list[SiteRunEntry], run_index_link: str, run_comparison_link: str) -> str:
    ...
    lines.extend(
        [
            ...
            "<th>Diagnostic Blockers</th><th>Capture Fails</th><th>Plan Blockers</th><th>Plan Warnings</th><th>Risk Flags</th>"
    ...
    for entry in entries:
        ...
        lines.append(
            ...
            f"<td>{entry.failure_diagnostics_blocker_count}</td>"
            f"<td>{entry.capture_manifest_fail_count}</td>"
            f"<td>{entry.real_run_plan_blocker_count}</td>"
            f"<td>{entry.real_run_plan_warning_count}</td>"
