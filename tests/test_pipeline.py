...
def test_run_scene_pipeline_dry_run_with_existing_config(tmp_path: Path) -> None:
    ...
    assert (tmp_path / "pipeline_runs" / "unit_scene" / "failure_diagnostics.json").exists()
    assert (tmp_path / "pipeline_runs" / "unit_scene" / "failure_diagnostics.md").exists()
...
def test_run_scene_pipeline_writes_run_scoped_demo_and_evaluation(tmp_path: Path) -> None:
    ...
    assert "failure_diagnostics.md" in (run_dir / "portfolio_page.html").read_text(encoding="utf-8")
    ...
    assert diagnostics_step.outputs["markdown"] == str(run_dir / "failure_diagnostics.md")
    diagnostics_payload = json.loads((run_dir / "failure_diagnostics.json").read_text(encoding="utf-8"))
    assert diagnostics_payload["status"] == "clear"
