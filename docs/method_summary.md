...
Each run can also produce `run_result_card.md` and `run_result_card.json`, a one-page reviewer-facing summary. It turns the same artifacts into a primary takeaway, shareable blurb, evidence snapshot, metrics, caveats, safe claims to avoid, readiness checks, and next actions. The card is calibrated by the submission packet and query-evidence audit: unresolved query-risk flags or failed query evidence block portfolio-ready status even when the scorecard is otherwise strong. The static `portfolio_page.html` mirrors the same result status and query-risk counts in its first-screen review summary, and the project-level run index, comparison reports, and experiment matrix use result/submission/query-evidence status for multi-run triage before selecting a portfolio candidate. This is meant to help a professor or recruiter quickly understand what the run demonstrates before opening the full research report.
