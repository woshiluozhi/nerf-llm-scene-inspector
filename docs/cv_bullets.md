...
- Designed reproducible CLI workflows for real-run preflight checks, data processing, scene-data validation, baseline NeRF training, language-field training, query visualization, experiment-matrix ablations, run-scoped evaluation, research-report generation, and portfolio-ready export.
...
- Built CPU-safe reproducibility infrastructure including dry-run training/query simulation, CI checks, capture manifests, real-run preflight reports, processed-scene quality reports, experiment-matrix summaries, paper-style research reports, evidence scorecards, project/run-level static portfolio pages, run-scoped portfolio packs, and professor-facing documentation.
